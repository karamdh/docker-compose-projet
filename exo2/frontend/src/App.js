...
export default function App() {
  ...
  async function updateUser(e) {
    e.preventDefault();
    try {
      await api(`/api/users/${encodeURIComponent(uUser)}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function deleteUser(e) {
    e.preventDefault();
    try {
      await api(`/api/users/${encodeURIComponent(dUser)}`, { method: "DELETE" });
      setDUser("");
      await refresh();
    } catch (e2) {
      setStatus(`Delete: ${e2.message}`);
    }
  }

  return (
    <div style={{ fontFamily: "sans-serif", padding: 24, maxWidth: 900 }}>
      <h1>Exo2 — SQLite CRUD Users</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
        ...
        <form onSubmit={updateUser} style={{ border: "1px solid #ddd", padding: 12, borderRadius: 8 }}>
          <h2>Update</h2>
          <input placeholder="username" value={uUser} onChange={(e)=>setUUser(e.target.value)} style={{ width:"100%", marginBottom: 8 }} />
        ...
        <form onSubmit={deleteUser} style={{ border: "1px solid #ddd", padding: 12, borderRadius: 8 }}>
          <h2>Delete</h2>
          <input placeholder="username" value={dUser} onChange={(e)=>setDUser(e.target.value)} style={{ width:"100%", marginBottom: 8 }} />
